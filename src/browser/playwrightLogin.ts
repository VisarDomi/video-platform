...
async function findAndClickLoginButton(page: Page, account: types.Account): Promise<void> {
...
async function handleGoogleLogin(popup: Page, account: types.Account): Promise<void> {
    const maxRetries = 3;
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
            ...
            // Required continue/consent step
...
async function extractLoginTokensFromResponse(response: any, accountEmail: string): Promise<types.LoginResult> {
...
export async function extractTokens(account: types.Account): Promise<types.LoginResult> {
    ...
    try {
        ...
        const errorMessage = `Failed to extract initial tokens via Playwright for ${account.email}.`;
