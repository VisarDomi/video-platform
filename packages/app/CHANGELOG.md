...
- **feature**: Ownership-based scroll sync — eliminate snap/jump when swiping back from player to list
  ...
  - **files**: `stores/player.svelte.ts` (scrollAnchorRatio, scrollTarget, captureScrollAnchor, updateScrollTarget, cleanup in showList), `routes/videos/[provider]/+page.svelte` (capture anchor in handleVideoClick, executor $effect, simplified view-transition effect, deleted scrollToActiveVideo), `components/VideoPlayer.svelte` (updateScrollTarget call in navigate callback)
