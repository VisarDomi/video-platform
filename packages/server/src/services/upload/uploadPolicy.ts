...
export function deriveSharedUploadPolicy(
    ...
    if (policies.length === 0) {
        throw new Error("At least one upload provider policy is required");
    }
    ...
    return {
        ...
        minimumDurationSeconds: minimumDurations.length > 0
            ? Math.max(...minimumDurations)
            : null,
        maximumDurationSeconds: maximumDurations.length > 0
            ? Math.min(...maximumDurations)
            : null,
